...
<<renameColumn, eval=FALSE>>=
colnames(maskData) <- c("Date", "Cases")
@
...
<<formatDate>>=
maskData$Date <- as.Date(maskData$Date, "%m/%d/%Y")
@
...
<<simple, tidy=FALSE>>=
fluData_subset <- subset(maskData, Date<as.Date("2011-08-14"))
...
fluData_singleSeason <- subset(maskData, 
...
alert_eval <- robustALERT(data=fluData_subset, allThresholds=TRUE,
...
colnames(alert_eval) <- c("rule", "thresh", "total cases", "dur", "ALERT cases", "%", "peaks", "peaks+/-k", "low weeks", "diff" )
